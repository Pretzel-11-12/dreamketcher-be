...
public interface EpisodeReportNotificationRepository extends
    ...
    // 모든 신고 알림 조회
    @Query("SELECT n FROM EpisodeLikeNotification n WHERE n.authorId = :memberId AND n.expiredAt > :dateTime ORDER BY n.createdAt DESC")
    List<EpisodeReportNotification> findAllNotificationsByMemberId(Long memberId,
        LocalDateTime dateTime);

    // 읽지 않은 신고 알림 조회
    @Query("SELECT n FROM EpisodeLikeNotification n WHERE n.authorId = :memberId AND n.isRead = false AND n.expiredAt > :dateTime ORDER BY n.createdAt DESC")
    List<EpisodeReportNotification> findUnreadNotificationsByMemberId(Long memberId,
        LocalDateTime dateTime);

    // 읽지 않은 알림 수 조회
    @Query("SELECT COUNT (n) FROM EpisodeLikeNotification n WHERE n.authorId = :memberId AND n.isRead = false AND n.expiredAt > :dateTime")
    Long countUnreadNotificationsByMemberId(Long memberId, LocalDateTime dateTime);

    // 만료된 신고 알림 조회
    @Query("SELECT n FROM EpisodeLikeNotification n WHERE n.expiredAt <= :dateTime")
